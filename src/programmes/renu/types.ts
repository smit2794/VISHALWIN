...
export interface SchoolAdmissionDetails {
  ...
  schoolName?: string;
  schoolType?: 'Government' | 'Private' | 'Inclusive' | 'Special School' | 'Other' | 'Home Education';
...
export interface CampTeamMember {
  id: string;
  role: 'Pediatrician' | 'Neurologist' | 'Therapist' | 'Volunteer' | 'Coordinator';
  name: string;
  mobile: string;
}
...
export interface AssessmentRecord {
  type: 'IQ' | 'Functional' | 'Behaviour' | 'Motor';
  date: string;
  toolUsed?: string;
  score?: number;
  finding?: string;
...
export interface IEPRecord {
  ...
  planPeriodTo: string;
  shortTermGoals: { goal: string; achieved: boolean }[];
  longTermGoals: { goal: string; achieved: boolean }[];
  quarterlyReviews: { date: string; reviewedBy: string; achievementPercent: number; remarks: string }[];
