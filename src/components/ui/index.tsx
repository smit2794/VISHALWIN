...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className = '', error, ...props }, ref) => {
    ...
    const paddingClass = hasHeight ? (className.includes('py-') ? '' : 'py-1 px-3') : 'px-4 py-2.5';
    return (
      <div className="w-full">
        <input
          ref={ref}
          className={`w-full ${paddingClass} text-sm bg-white text-slate-800 border rounded-xl leading-normal focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all ${
  ...
  ({ className = '', error, ...props }, ref) => {
    return (
      <div className="w-full">
        <textarea
          ref={ref}
          className={`w-full px-4 py-2.5 text-sm bg-white text-slate-800 border rounded-xl leading-normal focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all ${
            error ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500' : 'border-slate-200 hover:border-slate-400'
          } ${className}`}
          {...props}
        />
...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className = '', error, options, ...props }, ref) => {
    const hasHeight = className.includes('h-');
    const paddingClass = hasHeight ? (className.includes('py-') ? '' : 'py-1 pl-3 pr-8') : 'pl-4 pr-8 py-2';
    return (
      <div className="w-full">
        <select
          ref={ref}
          className={`w-full ${paddingClass} text-sm bg-white text-slate-800 border rounded-xl leading-normal focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22none%22%3E%3Cpath%20d%3D%22M7%209l3%203%203-3%22%20stroke%3D%22%2364748b%22%20stroke-width%3D%221.5%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem] bg-[right_0.5rem_center] bg-no-repeat ${
            error ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500' : 'border-slate-200 hover:border-slate-400'
          } ${className}`}
          {...props}
        >
